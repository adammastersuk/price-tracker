...
export function PricingStatusChip({ status }: { status: PricingStatus }) {
  const color: Record<PricingStatus, string> = {
    ...
    "In line with competitor": "bg-slate-100 text-slate-700 dark:bg-slate-600/80 dark:text-slate-50",
...
export function WorkflowChip({ status }: { status: WorkflowStatus }) {
  const color: Record<WorkflowStatus, string> = {
    ...
    "No Action": "bg-slate-200 text-slate-700 dark:bg-slate-600/85 dark:text-slate-50",
    Closed: "bg-slate-800 text-white dark:bg-slate-500 dark:text-slate-50",
