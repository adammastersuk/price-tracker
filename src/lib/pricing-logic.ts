...
export function derivePricingStatus(row: Pick<TrackedProductRow, "competitorCurrentPrice"|"competitorPromoPrice"|"competitorStockStatus"|"priceDifferencePercent">, inLineTolerancePercent = IN_LINE_TOLERANCE_PERCENT): PricingStatus {
  ...
  if (Math.abs(row.priceDifferencePercent) <= inLineTolerancePercent) return "In line with competitor";
